...
``mesh_demo.observations.records.record_from_dict``, so the coordinator-owned
...
#: ``mesh_demo.units`` refuses any unit that is not on a declared ladder.
...
class StationSample:
    ...
    ``mesh_demo.units``; the unit of each is fixed by :data:`CONTEXT_UNITS`.
    """
...
class ObservationGenerator:
    """Emit valid :class:`~mesh_demo.contracts.ObservationRecord` streams.
    ...
    def media_assay_record(
        ...
        *,
        ...
        active now (see ``mesh_demo.observations.operator``).
        """

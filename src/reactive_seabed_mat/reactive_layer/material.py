...
:class:`~reactive_seabed_mat.contracts.ProvenanceLabel`.  The defaults inherited from
...
accepted, and they delegate to ``reactive_seabed_mat.units``; no conversion factor is
re-implemented here.
"""
...
def contact_concentration_kg_per_m3(value: float, unit: str) -> float:
    """Display aqueous concentration -> SI, by delegation to ``reactive_seabed_mat.units``.

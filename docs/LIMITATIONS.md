...
The [numerical audit data](../manuscript/data/reactive_numerical_audit.json), [timescale studies](TIMESCALES.md) and manuscript give the errors and sensitivities actually measured. A convergent benchmark does not establish a precise field prediction, and a phase-dependent plume peak must not be described as horizon-independent.
...
- Censoring support in stored records and the standalone interval estimator does not establish its use in the coupled control loop. Current scalar QC skips the range check for censored records and cannot spike-check a bound without a value; when no check applies it assigns `NOT_EVALUATED` (2). The coupled estimator admits only `PASSED` (1). Generated non-detects and above-range records therefore remain archived evidence but are excluded from reconstruction unless they retain a passed post-QC flag. No validated bound-aware QC rule is implemented. In the regenerated one-year fresh-mat scenario, Hg chamber bounds at days 1 and 181 become flag 2; the quantified day-361 result arrives on day 382. The final snapshot consequently has no usable Hg chamber age and attenuation [0,1], and the policy records twelve sampling requests.
- For records admitted to the standalone interval estimator, above-range measurements preserve an unbounded upper interval. Finite loading bounds may come from the assumed capacity ceiling. Strict JSON uses `null` with `_nonfinite_values` path metadata for non-finite endpoints; `null` alone must not be interpreted as a measured zero. This export capability does not bypass the coupled QC limitation above.
...
- Stations assigned to absent tiles are omitted, not relocated. Four-tile scenario F lacks the default chamber station's `tile_2_2`, so it produces no Pb or Hg chamber records. Its 48 sampling requests and zero services reflect this monitoring-layout limitation; they do not establish chemical health of the undersized mat.
